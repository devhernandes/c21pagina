...
export function DiscoverSection() {
  ...
  const isImageLoaded = useSequentialLoad(imageRef, 0)
  const isContentLoaded = useSequentialLoad(contentRef, 200)
    ...
    <section className="py-16 px-4 md:px-8 lg:px-16 bg-[#F1EFEB]">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
        <motion.div 
          ...
          animate={isImageLoaded ? { opacity: 1, x: 0 } : { opacity: 0, x: 50 }}
        ...
        <motion.div 
          ...
          initial={{ opacity: 0, x: -50 }}
          animate={isContentLoaded ? { opacity: 1, x: 0 } : { opacity: 0, x: -50 }}
          transition={{ duration: 0.8 }}
        >
